  ...
  return (
    ...
    <nav className={`navbar navbar-expand-sm ${NavColor} ${BgContent}`}>
        <div className="container-fluid">
            {valid_Token ? <label className={`nav nav-link ${textColorPrimary}`} htmlFor="btn-sidebar"><MenuIcon/></label> : ""}
            <Link className="navbar-brand" to={valid_Token ? "#": "/"}>{valid_Token ? ` ${users.nombres}` : `Inicio`}</Link>
            ...
            <ul className="navbar-nav me-auto my-2 my-lg-0 navbar-nav-scroll ul-nav" >
                ...
                {!valid_Token ?  <Link className="nav-link active" aria-current="page" to={"/login"}>Login</Link> : ""}
                </li>
                ...
                {valid_Token ? <><center><p style={{fontSize: "8px"}} className={`${textColorPrimary}`}><PerfilIcon/><br/> {` ${users.nombres}`} </p></center></> : `Inicio`}
            </div>
        </div>
    </nav>
</>
